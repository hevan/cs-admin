...
export class MenuListComponent implements OnInit {
    ...
    paginateQuery(event) {
        ...
        this.menuService.pageQuery(paramsContract).subscribe(res=>{
            if(res.code == 0){
    ...
    toView(id){
        this.router.navigate(['/sys/menu/menuview', { 'id': id}]);
    }

    toEdit(id){
        this.router.navigate(['/sys/menu/menuedit', { 'id': id}]);
    }
